...
if [ "$MODE" != "bal" ]; then
  ...
  # Unit.speed 后必须重跑。
  run Creep
  # 战场法术与状态效果（M5 + M6）：锁死「dot 按固定间隔结算、不随帧率漂移」
  # 「心灵风暴敌我不分」「黑暗虫群只挡远程且只保护地面」
  # 「同源效果不叠加」「效果与法术区域能跨快照往返」五条铁律，
  # 以及 M6 的两条：「诱捕网是一次性施法、效果跟着单位走」
  # 「诱捕网同时降移速（×0.5）和降射速（武器冷却 ×1.25）」。
  #
  # ⚠️ 本套上线时抓到四个真问题：
  ...
  #
  # ⚠️ M6 追加的两条坑（都在测试侧，代码本身是对的）：
  #    5. 「持久区域对照」那条断言在施法后**没有推进一帧**就查效果 ——
  #       持久区域是靠 `_tick_spell_zones` 每帧挂上去的，施法完直接查必然是空的，
  #       于是「走出圈就失去免疫」**恒真**，整条镜像对照什么都没证明。
  #       （诱捕网是一次性施法，不需要这一步 —— 这正是两种机制的区别。）
  #    6. 「时长到期」那条原本用**虫后（空中）**当靶子，把「时长」和「对空」
  #       两件事混在一条断言里 —— 阳性对照打红时看不出是哪条规则破了。
  #       改成地面零伤害单位（蝎子）。
  # 改 Unit.effects / Unit.cooldown_time / World._tick_spell_effects / cmd_ability /
  # _apply_instant_aoe / _ai_ensnare_spot / blocked_by_dark_swarm /
